// Selectează elementul .page-container
const pageContainer = document.querySelector('.page-container');

// Funcția pentru a schimba poziția la apăsarea butonului de căutare
function changePageContainerPosition() {
    // Verifică dacă .page-container are deja clasa 'moved'
    const isMoved = pageContainer.classList.contains('moved');

    // Dacă este deja mutat, schimbă poziția la valoarea implicită (0)
    if (isMoved) {
        pageContainer.classList.remove('moved');
    } else {
        // Dacă nu este mutat, schimbă poziția la 20% din dreapta și adaugă clasa 'moved'
        pageContainer.style.right = '20%';
        pageContainer.classList.add('moved');
    }
}


// Adaugă un ascultător de eveniment pentru butonul de căutare
const searchBtn = document.querySelector('.btn-search');
searchBtn.addEventListener('click', changePageContainerPosition);
...
const todayWeatherPanel = document.querySelector('.weather-panel.today');
